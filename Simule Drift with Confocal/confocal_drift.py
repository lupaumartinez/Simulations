...
def matrix_time(exposure_time, number_pixel, confocal_type):
    ...
    print(matrix_pixel_time.T)

    return matrix_pixel_time.T

def multiple_images(number_pixel, rango, xo, yo, vx, vy, exposure_time, confocal_type):
...
def image_map_drift(number_pixel, rango, xo, yo, vx_max, vy_max, N, exposure_time, confocal_type):
	...
	for i in range(len(vx)):
		for j in range(len(vy)):
			...
			stack_image, first_position, last_position = multiple_images(number_pixel, rango, xo, yo, vx[i], vy[j], exposure_time, confocal_type)
...
path_to = os.path.join(parent_folder,'Simulation_drift_on_confocal_spiral')
...
position_NP, first_position_NP, last_position_NP = multiple_images(number_pixel = number_pixel, rango = rango, xo = 0, yo = 0,
                      vx = vx_drift, vy = vy_drift, exposure_time = exposure_time, confocal_type = confocal_type)
...
vx, vy, stack_adquisition =  image_map_drift(number_pixel = number_pixel, rango = rango, xo = 0, yo = 0, 
                                             vx_max = vx_drift, vy_max = vy_drift, N = Number_velocity, exposure_time = exposure_time,  confocal_type = confocal_type)
vo_module = []
...
for i in range(Number_velocity):
    for j in range(Number_velocity):
        ...
        print('xo', xo, 'yo', yo, 'vx' , v_x, 'vy', v_y)
